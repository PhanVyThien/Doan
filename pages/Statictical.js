...
export default function Statictical({data}){
    ...
    return (
        <Mother>
            <Buttonscontaner>
                <Showblock color={'red'} data={{time:'doanh thu tháng này',value:total(data.This_month_Bills),count:Count(data.This_month_Bills)}}/>
                <Showblock color={'blue'} data={{time:'doanh thu tháng trước',value:total(data.Last_month_Bills),count:Count(data.Last_month_Bills)}}/>
                <Showblock color={'green'} data={{time:'doanh thu hôm nay',value:total(data.Today_Bills),count:Count(data.Today_Bills)}}/>
                <Showblock color={'yellow'} data={{time:'Tổng doanh thu',value:total(data.Total_Bills),count:Count(data.Total_Bills)}}/>
            </Buttonscontaner>
            <Devide>
                <tr>
                    <td>
                        <Table data={{name:'Sản phẩm',header:['Tên sản phẩm','Giá đơn vị','Số lượng bán','Tổng'],rows:data.Pros_Pre}}/>
                    </td>
                    <td>
                        <Table data={{name:'Khách hàng',header:['Tên khách hàng','Số hàng đã mua','Tổng tiêu'],rows:data.Users_Pre}}/>
                    </td>
                </tr>
    ...
    for(var i=0;i<bills.length;i++){
        for(var j=0;j<bills[i].Products.length;j++){
            if((bills[i].Products)[j].product){
                ...
                re[(bills[i].Products)[j].product.id][3]+=re[(bills[i].Products)[j].product.id][1];
            }
            if(re[(bills[i].Products)[j].courseId]==undefined){
                continue;
            }
            re[(bills[i].Products)[j].courseId][2]++;
            re[(bills[i].Products)[j].courseId][3]+=re[(bills[i].Products)[j].courseId][1];
...
Statictical.getInitialProps = async (ctx) =>{
    ...
    const resThismonthBills= await fetch("http://localhost:5035/bills/dateRange?startDate="+This_month_Bills_Start_date.toLocaleDateString()+"&endDate="+Now );
    ...
    Last_month_Bills_Start_date.setMonth(Last_month_Bills_Start_date.getMonth()-1);
    var Last_month_Bills_End_date=new Date(Last_month_Bills_Start_date.toLocaleDateString());
    Last_month_Bills_End_date.setMonth(Last_month_Bills_End_date.getMonth()+1);
    Last_month_Bills_End_date.setDate(0);
    const res_Last_month_Bills= await fetch("http://localhost:5035/bills/dateRange?startDate="+Last_month_Bills_Start_date.toLocaleDateString()+"&endDate="+Last_month_Bills_End_date.toLocaleDateString() );
    ...
    var Today=new Date();
    const res_Today_Bills= await fetch("http://localhost:5035/bills/dateRange?startDate="+Today.toLocaleDateString()+"&endDate="+Today );
    ...
    return {data:{This_month_Bills:jsonThismonthBills,
